...
{if $post['first']}
<div class="zhanzhuai_viewthread" style="border-top: 1px solid #eee; padding-top: 20px;">
...
<td class="plc plm">
		...
		<!--{if $post['relateitem']}-->
			<div class="mtw mbw">
				<h3 class="pbm mbm bbda" style="font-size: 16px;font-weight: 400;">{lang related_thread}</h3>
				<ul class="xl xl2 cl">
...
<div class="zhanzhuai_viewtitle">
	<h4>已有($_G[forum_thread][allreplies])人评论</h4>
...
<div class="zhanzhuai_replythread">
<table id="pid$post[pid]" class="table_reply plhin" summary="pid$post[pid]" cellspacing="0" cellpadding="0">
<tr>
	<!--{if !$close_leftinfo}-->
		...
		<div id="favatar$post[pid]" class="favatar">
			...
			<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
				...
				<!--{if !$postshowavatars}-->
					<div class="avatar_ico">{lang member_avatar_banned}</div>
				<!--{elseif $post['avatar'] && $showavatars}-->
					...
					<!--{/if}-->
					<div class="avatar_ico"{if !($_G['setting']['threadguestlite'] && !$_G['uid'])} onmouseover="showauthor(this, 'userinfo$post[pid]')"{/If}><a href="home.php?mod=space&uid=$post[authorid]" class="avtm" target="_blank">$post[avatar]</a></div>
				<!--{/if}-->
	...
	<!--{/if}-->
	<td class="plc plct"<!--{if $close_leftinfo}--> style="width:100%"<!--{/if}-->>
		<div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:48px"<!--{/if}-->>

		<!--{if $close_leftinfo}--><div class="zhanzhuai_cleft_avatar"><a href="home.php?mod=space&uid=$post[authorid]" target="_blank">$post[avatar]</a></div><!--{/if}-->
			...
			<!--{if !IS_ROBOT}-->
				...
				<!--{/if}-->
				<strong>
					<a style="border: none;background: {if $post[number] == 2}#ff4c4c{elseif $post[number] == 3}#ff8b3d{elseif $post[number] == 4}#6999ee{else}transparent{/if};color: {if $post[number] == 2}#fff{elseif $post[number] == 3}#fff{elseif $post[number] == 4}#fff{else}#999{/if};height: {if $post[number] == 2}20px{elseif $post[number] == 3}20px{elseif $post[number] == 4}20px{else}20px{/if};line-height: {if $post[number] == 2}20px{elseif $post[number] == 3}20px{elseif $post[number] == 4}20px{else}20px{/if};{if $close_leftinfo}margin-top: 14px;{else}margin-top: 4px;{/if}padding: {if $post[number] == 2}0 5px{elseif $post[number] == 3}0 5px{elseif $post[number] == 4}0 5px{else}0px{/if}" href="{if $post[first]}forum.php?mod=viewthread&tid=$_G[tid]$fromuid{else}forum.php?mod=redirect&goto=findpost&ptid=$_G[tid]&pid=$post[pid]$fromuid{/if}" {if $fromuid}title="{lang share_url_copy_comment}"{/if} id="postnum$post[pid]" onclick="setCopy(this.href, '{lang post_copied}');return false;">
			...
			<!--{/if}-->
			<div class="pti" {if $close_leftinfo}style="padding-top: 10px;"{/if}> 
				<div class="pdbt">
...
<tr>
	...
	<!--{/if}-->
	<td class="plc plcb" style="overflow:visible;{if $close_leftinfo}--> width:100%{/if}">
		<div class="po hin">
            <!--{if !$post['first'] && $modmenu['post']}-->
				<span class="y">
				<label for="manage$post[pid]">
