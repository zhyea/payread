...
<!--{if empty($_GET[from])}-->
	...
	<div class="cl">
		<div{if empty($_GET[from]) && $_G[setting][fastsmilies]} class="hasfsl"{/if} id="fastposteditor">
			<div class="tedt{if !($_G[forum_thread][special] == 5 && empty($firststand))} {/if}" style=" border: 0">
				...
				<div class="bar" style=" background: #fff; border-bottom: 0;padding: 0;">
					...
					<span class="y">
						...
						<a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]{if $_GET[from]}&from=$_GET[from]{/if}" onclick="return switchAdvanceMode(this.href)" style="color: #888;">使用高级模式，上传图片！</a>
					</span>
				...
				<div class="area">
					<!--{if $allowfastpost}-->
						<textarea rows="6" cols="80" name="message" id="fastpostmessage" onKeyDown="seditor_ctlent(event, {if $_GET['ordertype'] != 1}'fastpostvalidate($(\'fastpostform\'))'{else}'$(\'fastpostform\').submit()'{/if});" tabindex="4" placeholder="说点什么..." class="pt"{eval echo getreplybg($_G['forum']['replybg']);}></textarea>
					<!--{else}-->
...
<div class="ptm pnpost" style="height: 35px; line-height: 35px; margin: 10px 0 0;padding-top: 0 !important;overflow: hidden;">
    <div class="zhanzhua_post_yz z">
	    <!--{eval $seditor = array('fastpost', array('bold', 'color', 'link', 'quote', 'code', 'smilies'));}-->
